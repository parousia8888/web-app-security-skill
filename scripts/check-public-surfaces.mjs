...
if (contract.candidateSourceRelease?.version !== '0.7.0'
    || contract.candidateSourceRelease?.status !== 'candidate_unpublished'
    || !existsSync(`${ROOT}/${contract.candidateSourceRelease?.routeSchema || ''}`)
    || !existsSync(`${ROOT}/${contract.candidateSourceRelease?.accessControlReview || ''}`)
    || !existsSync(`${ROOT}/${contract.candidateSourceRelease?.accessControlRegressions || ''}`)) {
  fail('v0.7.0 candidate access-control contract is missing or invalid');
}
if (JSON.stringify(contract.candidateSourceRelease?.stableFrameworks) !== JSON.stringify([
  'express', 'nestjs', 'next-app',
])) fail('v0.7.0 candidate framework scope changed');
