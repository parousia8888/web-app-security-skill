...
Status: release candidate. Phases 0-6 of the external-audit remediation plan are complete. The final
local candidate gate, exact-commit CI/CodeQL, signed tag, GitHub Release, npm provenance, verified
installer, immutable Action consumer, signed `v1` promotion and durable live-verification record
remain gated. Published v0.7.2 facts stay in `docs/release-state.json` until each v0.7.3 channel is
...
- Five ordinary-project journeys use one pinned active contract with separate byte, deterministic
  semantic, mutable-advisory and manual-annotation identity. Historical v0.6.0 review bytes remain
  immutable and current-analyzer reproduction remains `verification_pending` where raw invocation
...
- Journey tests cover exit 0/1/2/3, prerequisite drift, deterministic versus mutable digests,
  Gitleaks redaction collisions and five exact target commits.
...
plant-the-failure check in its owning phase. The full repository gate is intentionally deferred
until the candidate tree stabilizes.

## Candidate verification

Pending final candidate freeze:
...
SSH-signed annotated `v0.7.3` tag exists. The tag triggers reproducible archive, SPDX SBOM,
checksums, release manifest, GitHub attestation and GitHub Release publication. npm trusted
publishing, installer/bootstrap trust updates, immutable Action and signed `v1` promotion remain
