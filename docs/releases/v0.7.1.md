...
- The four observed release digests are now recorded in the verifier trust map, whose default is
  `0.7.1`. The stable Action remains recorded at v0.7.0 until the immutable v0.7.1 consumer passes;
  bootstrap and README pins remain on their prior verified identities until the next trust-chain
  gates complete.
...
- The README bootstrap and immutable Action examples now select those public v0.7.1 identities.
  Their publication commit and bounded consumers remain gated before `v1` promotion.

Verified-installer pinning, exact-version installation, immutable Action consumption and signed
`v1` read-back will be appended only after their public gates pass. Artifact identity and
provenance do not prove every detector conclusion correct or an audited project secure.
